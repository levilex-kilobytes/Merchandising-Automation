...
async function main(): Promise<void> {
  ...
  app.use(
    config.API_PREFIX,
    requireFeature('FEATURE_INVENTORY', config.FEATURE_INVENTORY),
    apiRouter,
  );
  ...
  const inventory = mfa['inventory'] as Record<string, unknown> | undefined;
  if (!inventory) throw new Error('Proto package "mfa.inventory" not found');
  const v1 = inventory['v1'] as Record<string, unknown> | undefined;
  if (!v1) throw new Error('Proto package "mfa.inventory.v1" not found');
  const InventoryService = v1['InventoryService'] as { service: grpc.ServiceDefinition } | undefined;
  if (!InventoryService) throw new Error('InventoryService not found in proto');

  const server = new grpc.Server();
  server.addService(
    InventoryService.service,
    buildInventoryGrpcHandlers(stockService) as never,
  );
  server.bindAsync(
