...
export class StockRepository {
  ...
  async upsertStockItem(
    ...
    const { rows } = await tx.query<StockItemRow>(
      `INSERT INTO stock_items
         (product_code, product_name, location_code, unit_cost, low_stock_threshold)
       VALUES ($1, $2, $3, $4, $5)
  ...
  async incrementAllocated(tx: PoolClient, id: string, delta: number): Promise<StockItem> {
    const { rows } = await tx.query<StockItemRow>(
      `UPDATE stock_items
  ...
  async incrementOnOrder(tx: PoolClient, productCode: string, locationCode: string, delta: number): Promise<void> {
    await tx.query(
      `UPDATE stock_items
       SET on_order = GREATEST(0, on_order + $1), updated_at = NOW()
       WHERE product_code = $2 AND location_code = $3`,
      [delta, productCode, locationCode],
    );
  }
  ...
  async recordMovement(
    ...
    const { rows } = await tx.query<MovementRow>(
      `INSERT INTO stock_movements
         (product_code, location_code, movement_type, quantity, reference_id, reference_type, notes)
  ...
  async findByProductAndLocation(
    ...
    const { rows } = await client.query<StockItemRow>(
      `SELECT * FROM stock_items WHERE product_code = $1 AND location_code = $2`,
      [productCode, locationCode],
    );
    return rows[0] ? toStockItem(rows[0]) : null;
  }

  async findById(id: string, tx?: PoolClient): Promise<StockItem | null> {
    const client = tx ?? pool;
    const { rows } = await client.query<StockItemRow>(
  ...
  async list(filter: ListStockQueryDto, lowThreshold: number): Promise<StockItem[]> {
    ...
    if (filter.locationCode) {
      params.push(filter.locationCode);
      conditions.push(`location_code = $${params.length}`);
    }
    if (filter.lowOnly) {
      params.push(lowThreshold);
      conditions.push(`on_hand <= $${params.length}`);
    }
    const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
    const { rows } = await pool.query<StockItemRow>(
      `SELECT * FROM stock_items ${where} ORDER BY product_code, location_code`,
      params,
    );
  ...
  async listMovements(productCode?: string, locationCode?: string): Promise<StockMovement[]> {
    ...
    const { rows } = await pool.query<MovementRow>(
      `SELECT * FROM stock_movements ${where} ORDER BY created_at DESC LIMIT 100`,
      params,
    );
  ...
  async listLocations(): Promise<string[]> {
    const { rows } = await pool.query<{ location_code: string }>(
      `SELECT DISTINCT location_code FROM stock_items ORDER BY location_code`,
    );
