...
export interface StockMovement {
  ...
  referenceId: string | null; referenceType: string | null; notes: string | null; createdAt: Date;
}
export interface AdjustmentDto { productCode: string; locationCode: string; delta: number; reason: string; }
export interface ListStockQueryDto { productCode?: string; locationCode?: string; lowOnly?: boolean; }
export interface ReceiveStockInput {
  productCode: string; productName: string; locationCode: string;
  quantity: number; unitCost: number; referenceId: string; referenceType: string;
}
