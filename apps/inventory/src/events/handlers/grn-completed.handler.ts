...
interface GRNCompletedPayload {
  grnId: string; poId: string; supplierId: string; grnDate: string;
...
export async function registerGRNCompletedHandler(bus: EventBus, logger: Logger, service: StockService): Promise<void> {
  await bus.subscribe<GRNCompletedPayload>('receiving.grn.completed', async (event) => {
    ...
    try {
      if (!p.lines || p.lines.length === 0) { logger.warn('GRN without lines', { grnId: p.grnId }); return; }
      for (const line of p.lines) {
        await service.receiveStock({
          ...
          unitCost: line.unitCost, referenceId: p.grnId, referenceType: 'grn',
        });
      }
      logger.info('Stock received from GRN', { grnId: p.grnId, lineCount: p.lines.length });
    } catch (err) {
      logger.error('Failed to receive stock', { grnId: p.grnId, err: String(err) });
      throw err;
    }
  });
}
