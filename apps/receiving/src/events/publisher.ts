...
export class OutboxPublisher {
  ...
  constructor(private readonly cfg: OutboxPublisherConfig) {
    if (cfg.intervalMs <= 0) throw new Error('OutboxPublisher: intervalMs must be > 0');
    if (cfg.batchSize <= 0) throw new Error('OutboxPublisher: batchSize must be > 0');
  }

  start(): void {
    this.timer = setInterval(() => {
      this.flush().catch((err) =>
        this.cfg.logger.error('Outbox flush failed', { err: String(err) }),
      );
    }, this.cfg.intervalMs);
    this.cfg.logger.info('OutboxPublisher started', {
  ...
  async flush(): Promise<void> {
    ...
    for (const row of batch) {
      try {
        ...
        this.cfg.logger.error('Publish failed, will retry', { id: row.id, err: String(err) });
        break;
      }
    }
  }
